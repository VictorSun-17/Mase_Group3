...
def split_and_flatten_2d_tensor(input_tensor, row_block_size, col_block_size):
    """
    Split and flatten a 2D tensor into blocks with given row and column block sizes.
    """
...
class fixed_swish_tb(Testbench):
    def __init__(self, module, dut, dut_params, float_test=False) -> None:
        """
        Testbench for fixed_swish.
        :param module: Torch module implementing swish (e.g., torch.nn.SiLU() or a lambda function).
        :param dut: The device under test.
        :param dut_params: Dictionary of DUT parameters.
        :param float_test: Whether to use float monitor.
        """
        ...
        # Create stream driver for input interface
        self.data_in_0_driver = StreamDriver(
            dut.clk, dut.data_in_0, dut.data_in_0_valid, dut.data_in_0_ready
        )

        # Create stream monitor for output interface
        if float_test:
            ...
            self.data_out_0_monitor = StreamMonitor(
                dut.clk, dut.data_out_0, dut.data_out_0_valid, dut.data_out_0_ready, check=False
            )

        # Define quantizers for input and output data
        ...
        # Set the torch model for swish: x * sigmoid(x)
        self.model = module  # e.g., torch.nn.SiLU() or lambda x: x * torch.sigmoid(x)
        ...
        # Compute expected swish output using the torch model
    ...
    def exp(self):
        """
        Run the torch model with provided inputs and return the expected integer outputs
        in the format expected by the monitor.
        """
        # Split and flatten the output tensor to match DUT output format
    ...
    def generate_inputs(self):
        """
        Generate integer inputs for the DUT in the format expected by the driver.
        """
    ...
    def doubletofx(self, num, data_width, f_width, type="bin"):
        """
        Convert a floating point number to a fixed-point binary or hex string.
        """
        assert type in ["bin", "hex"], "type can only be: 'hex' or 'bin'"
    ...
    async def run_test(self):
        """
        Main test routine: reset DUT, generate inputs, drive them to DUT and compare outputs.
        """
        await self.reset()
        logger.info("Reset finished")
        self.data_out_0_monitor.ready.value = 1
        # Run several test iterations
        for i in range(10):
            inputs, _ = self.generate_inputs()
            exp_out = self.exp()
            inputs = inputs.tolist()
            exp_out = exp_out.tolist()

            logger.info("Inputs and expected outputs generated")
...
async def cocotb_test(dut):
    # Get DUT parameters from a global dut_params dictionary
    ...
    # Instantiate the testbench with a swish model.
    # Here we use torch.nn.SiLU which implements swish (x * sigmoid(x))
    tb = fixed_swish_tb(torch.nn.SiLU(), dut, dut_params, float_test=False)
    await tb.run_test()

# DUT parameters definition (example values)
dut_params = {
...
@pytest.mark.dev
def test_fixed_swish():
    # If needed, generate LUT or any auxiliary files for the swish function here.
    # For example, if your fixed_swish module uses a LUT for the sigmoid part, generate it:
    generate_memory.generate_sv_lut(
        "sigmoid",  # Assuming the same LUT is used internally by fixed_swish
    ...
    print("Generated memory for swish (sigmoid part)")
